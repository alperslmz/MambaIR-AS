...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y = self.out_norm(y)
        y = y * self._apply_gate_activation(z)
        out = self.out_proj(y)
...
class VSSBlock_ReLU(nn.Module):
    """VSSBlock variant that keeps CAB+CA intact but swaps SS2D activations to ReLU."""
...
class BasicLayer_ReLU(nn.Module):
    """BasicLayer variant that stacks VSSBlock_ReLU blocks."""
    ...
    def __init__(self, dim, input_resolution, depth, drop_path=0., d_state=16,
        ...
        self.blocks = nn.ModuleList()
        for i in range(depth):
            self.blocks.append(VSSBlock_ReLU(
                hidden_dim=dim,
                drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                norm_layer=nn.LayerNorm,
                attn_drop_rate=0,
...
class ResidualGroup_ReLU(nn.Module):
    """ResidualGroup using BasicLayer_ReLU while preserving CAB + ChannelAttention."""
    ...
    def __init__(self, dim, input_resolution, depth, d_state=16, mlp_ratio=4.,
        ...
        super(ResidualGroup_ReLU, self).__init__()
        self.dim = dim
        self.input_resolution = input_resolution

        self.residual_group = BasicLayer_ReLU(
            dim=dim,
            input_resolution=input_resolution,
            depth=depth,
            d_state=d_state,
...
@ARCH_REGISTRY.register()
class MambaIR_ReLU(nn.Module):
    r"""M4 ablation: retain CAB + ChannelAttention, replace SS2D SiLU activations with ReLU."""
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(MambaIR_ReLU, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        self.img_range = img_range
...
class ResidualGroup_MCSA_Group(nn.Module):
    """ResidualGroup with one group-level MCSA after the 6-block stack.

    Unlike block-level MCSA, this variant keeps the baseline VSSBlocks intact
    and applies a single spatial attention module to the 2-D feature map
    produced by the whole residual group before the final convolution.
    """
    ...
    def __init__(self, dim, input_resolution, depth, d_state=16, mlp_ratio=4.,
        ...
        self.spatial_attn = MambaSpatialAttn(dim)
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=0, embed_dim=dim, norm_layer=None)
        self.patch_unembed = PatchUnEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=0, embed_dim=dim, norm_layer=None)
    ...
    def forward(self, x, x_size):
        group_tokens = self.residual_group(x, x_size)
        group_2d = self.patch_unembed(group_tokens, x_size)
        sa_map = self.spatial_attn(group_2d)
        group_2d = group_2d * (1.0 + sa_map)
        group_2d = self.conv(group_2d)
        return self.patch_embed(group_2d) + x
...
@ARCH_REGISTRY.register()
class MambaIR_MCSA_Group(nn.Module):
    r"""MambaIR variant with one MCSA module per ResidualGroup.

    The baseline CAB + ChannelAttention path is preserved. Instead of inserting
    MCSA inside every VSSBlock, a single MambaSpatialAttn module is applied to
    the 2-D feature map produced by each full 6-block residual group.
    """
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(MambaIR_MCSA_Group, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        self.img_range = img_range
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = ResidualGroup_MCSA_Group(
                dim=embed_dim,
                input_resolution=(patches_resolution[0], patches_resolution[1]),
                depth=depths[i_layer],
                d_state=d_state,
